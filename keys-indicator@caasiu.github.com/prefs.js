...
const KeysPrefsWidget = new GObject.Class({
  ...
  _init: function(params) {
    ...
    this.prefKeys = new Gtk.Label({
      visible: true,
      label: _("Supported Keys: ")
    });
    this.prefCaps = new Gtk.Label({ visible: true, label: _("[CapsLock]") });
    //this.prefCaps.set_style('color: red; bindex: 1px solid red;');

    this.prefNum = new Gtk.Label({ visible: true, label: _("[NumLock]") });

    //put all the label together
    let keyLabels = new Gtk.Box({
    ...
    let introLabel = new Gtk.Label({
      label: _(
        "Please choose your favorite style and highlighting mode. You can directly test it, to see the differences!"
      ),
      wrap: true,
      xalign: 0
    });
    ...
    let styles = {
      popup: _("Pop-Up each"),
      less: _("Less disruptive"),
      grayout: _("Gray-Out unpressed")
    };
    for (name in styles) {
    ...
    let AttentionBox = new Gtk.Box({
      orientation: Gtk.Orientation.HORIZONTAL,
      margin: 10
    });
    let AttentionLabel = new Gtk.Label({
      label: _("Highlight if CAPS=on / NUM=off:"),
      xalign: 0
    });
    let AttentionWidget = new Gtk.ComboBoxText();
    let highlights = {
      "highlight-red": _("Highlight in red"),
      none: _("Disable")
    };
    ...
    let IndexBox = new Gtk.Box({
      orientation: Gtk.Orientation.HORIZONTAL,
      margin: 10
    });
    let IndexLabel = new Gtk.Label({
      label: _("Panel Position Index:"),
      xalign: 0
    });
